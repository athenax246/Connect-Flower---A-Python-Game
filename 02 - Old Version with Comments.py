...
XMARGIN = int((WINDOWWIDTH - BOARDWIDTH * SPACESIZE) / 2) #width - number of pixels = xmargin along sides
YMARGIN = int((WINDOWHEIGHT - BOARDHEIGHT * SPACESIZE) / 2) #height - number of pixels = ymargin along sides
...
BGCOLOR = BRIGHTBLUE #backgoround colour
...
def main():
  #makes all variables global which means those variables could be used anywhere in the code
    ...
    FPSCLOCK = pygame.time.Clock() #time clock for pygame, sets up frames per seconds
    DISPLAYSURF = pygame.display.set_mode((WINDOWWIDTH, WINDOWHEIGHT)) #displays window width and height
    pygame.display.set_caption('Four in a Row') #sets caption to be 'Four in a Row'

    REDPILERECT = pygame.Rect(int(SPACESIZE / 2), WINDOWHEIGHT - int(3 * SPACESIZE / 2), SPACESIZE, SPACESIZE) #creating rectangles for collision
    BLACKPILERECT = pygame.Rect(WINDOWWIDTH - int(3 * SPACESIZE / 2), WINDOWHEIGHT - int(3 * SPACESIZE / 2), SPACESIZE, SPACESIZE)
    REDTOKENIMG = pygame.image.load('4row_red.png') #loads in token for image
    REDTOKENIMG = pygame.transform.smoothscale(REDTOKENIMG, (SPACESIZE, SPACESIZE)) #transforms image into token size needed
    BLACKTOKENIMG = pygame.image.load('4row_black.png') #loads in token for image
    BLACKTOKENIMG = pygame.transform.smoothscale(BLACKTOKENIMG, (SPACESIZE, SPACESIZE)) #transforms image into token size needed
    BOARDIMG = pygame.image.load('4row_board.png') #loads in board for image
    BOARDIMG = pygame.transform.smoothscale(BOARDIMG, (SPACESIZE, SPACESIZE)) #transforms image into board size needed

    HUMANWINNERIMG = pygame.image.load('4row_humanwinner.png') #loading image
    COMPUTERWINNERIMG = pygame.image.load('4row_computerwinner.png') #loading image
    TIEWINNERIMG = pygame.image.load('4row_tie.png') #loading image
    WINNERRECT = HUMANWINNERIMG.get_rect() #creates rectangle for image
    WINNERRECT.center = (int(WINDOWWIDTH / 2), int(WINDOWHEIGHT / 2)) #creates rectangle for image

    ARROWIMG = pygame.image.load('4row_arrow.png') #loading image
    ARROWRECT = ARROWIMG.get_rect() #creates rectangle for image
    ARROWRECT.left = REDPILERECT.right + 10 #place rectangle for image in correct location
    ARROWRECT.centery = REDPILERECT.centery #centres rectangle for image
...
def runGame(isFirstGame):
    ...
    while True: # main game loop
        ...
        if isBoardFull(mainBoard):
            ...
            #if all tokens are filled up, it's a tie and it'll display the TIEWINNERIMG
...
def drawBoard(board, extraToken=None):
    ...
    for x in range(BOARDWIDTH):
        for y in range(BOARDHEIGHT): #nested for loops iterating each column and row from the game board
            spaceRect.topleft = (XMARGIN + (x * SPACESIZE), YMARGIN + (y * SPACESIZE)) #fit a rectangle to each space in the columns and rows, use ".topleft" to align them
    ...
    if extraToken != None: #if there should exist an extra token
        if extraToken['color'] == RED: #if the extra token from the human player
            DISPLAYSURF.blit(REDTOKENIMG, (extraToken['x'], extraToken['y'], SPACESIZE, SPACESIZE)) #draw an extra token image to the appropriate location on the display surface
        elif extraToken['color'] == BLACK: #if the extra token is from the computer
            DISPLAYSURF.blit(BLACKTOKENIMG, (extraToken['x'], extraToken['y'], SPACESIZE, SPACESIZE)) #draw an extra token image to the appropriate location on the display surface
...
def getHumanMove(board, isFirstMove):
    ...
    while True:
        for event in pygame.event.get(): #handling each event that we get from the queue
            if event.type == QUIT or (event.type == pygame.KEYUP and event.key == pygame.K_ESCAPE): #similar to before, close the game if the play clicks the [X] button of the game window or press the escape key
                pygame.quit()
                sys.exit()
        ...
        if tokenx != None and tokeny != None: #if both positions of the token have a value
            drawBoard(board, {'x':tokenx - int(SPACESIZE / 2), 'y':tokeny - int(SPACESIZE / 2), 'color':RED}) #Draws the token on the board after a location on the board has been decided
        else:
            drawBoard(board) #otherwise keep the board as is
...
def animateDroppingToken(board, column, color):
    ...
    while True:
        y += int(dropSpeed) #moves the token down the y-axis at the speed specified (1 pixel per 2 frames)
        dropSpeed += 0.5 #add 0.5 pixels to the drop speed every time this code is ran
        if int((y - YMARGIN) / SPACESIZE) >= lowestEmptySpace: #if the token object is at the lowest empty space,
            return #stop the animation
        drawBoard(board, {'x':x, 'y':y, 'color':color})
        pygame.display.update() #update the display of what has changed every frame
        FPSCLOCK.tick() #called every frame in order to ensure game is running at specified fps
...
def animateComputerMoving(board, column):
    ...
    while y > (YMARGIN - SPACESIZE): #while the token is still under the y-value where the token should be dropped
        ...
        drawBoard(board, {'x':x, 'y':y, 'color':BLACK}) #creates a token on the board at the position where the token was dropped
        pygame.display.update() #update the display of what has changed every frame
        FPSCLOCK.tick() #called every frame in order to ensure game is running at specified fps
    ...
    while x > (XMARGIN + column * SPACESIZE):
        ...
        speed += 0.5 #makes the pixels per frame moved get faster every frame
        drawBoard(board, {'x':x, 'y':y, 'color':BLACK}) #creates a token on the board at the position where the token was dropped
        pygame.display.update() #update the display of what has changed every frame
        FPSCLOCK.tick() #called every frame in order to ensure game is running at specified fps
    # dropping the black tile
    animateDroppingToken(board, column, BLACK) #Starts the dropping animation from where the token was moved
...
def getComputerMove(board):
    ...
    Args:
        board (list): The board used where the movement of token is animated (only needed for board that is displayed to user)
    """
    potentialMoves = getPotentialMoves(board, BLACK, DIFFICULTY) #calls the getPotentialMoves() function and uses the board to calculate all of black's possible moves, DIFFICULTY moves into the future
    # get the best fitness from the potential moves
    bestMoveFitness = -1 #default best move fitness is -1
    for i in range(BOARDWIDTH): #loop through this for all rows across the board
        if potentialMoves[i] > bestMoveFitness and isValidMove(board, i): #if the move is better than the default move fitness and is a valid move
            bestMoveFitness = potentialMoves[i] #make that the best move fitness
    # continue loop until the best move(s) are found      
    bestMoves = [] #create a list for all moves with the highest and same bestMoveFitness value
    for i in range(len(potentialMoves)): #for all potential moves
        if potentialMoves[i] == bestMoveFitness and isValidMove(board, i):
            bestMoves.append(i) #search through the possible moves and make add all the best moves (ones with the highest bestMoveFitness) into a list
    return random.choice(bestMoves) #select randomly between all of the best possible moves
...
def getPotentialMoves(board, tile, lookAhead):
    ...
    """
    if lookAhead == 0 or isBoardFull(board): #if there are no more moves to look at or the board is full
    ...
    potentialMoves = [0] * BOARDWIDTH #create a list with 0's the size of the rows
    for firstMove in range(BOARDWIDTH):
        dupeBoard = copy.deepcopy(board) #create a copy of the board
        if not isValidMove(dupeBoard, firstMove): #check if moves being checked are valid
        ...
        if isWinner(dupeBoard, tile):
          ...
          # a winning move automatically gets a perfect fitness
            potentialMoves[firstMove] = 1 # investigate the move fitness ranking system
            break # don't bother calculating other moves
        else: # if the move is not a winning move,
            # do other player's counter moves and determine best one
            if isBoardFull(dupeBoard): # if there are no empty spaces on the copy of the board
                ...
                for counterMove in range(BOARDWIDTH): # for every available space in the width of the board, consider:
                    dupeBoard2 = copy.deepcopy(dupeBoard) # make a copy of the board to analyze
                    if not isValidMove(dupeBoard2, counterMove): # Is the space you're looking at available to play a piece?
                        continue # if it is not available, go to the next space.
                    makeMove(dupeBoard2, enemyTile, counterMove) # the computer pretends the player makes a move in its dupeBoard2
                    if isWinner(dupeBoard2, enemyTile): # If the player's tile would win,
                        ...
                        results = getPotentialMoves(dupeBoard2, tile, lookAhead - 1) # if the computer can't win from looking at the next 2 potential moves,
...
def getLowestEmptySpace(board, column):
    # asks what space the chip can be placed in a certain column
    ...
    for y in range(BOARDHEIGHT-1, -1, -1): # start at the upper limit of the board and keep checking down in increments of 1 until the last space
        if board[column][y] == EMPTY: # if the coordinate indicated on the board is empty,
            return y # return the y coordinate that is empty
    return -1


def isValidMove(board, column): # tells where you or the computer can put a chip
    # Returns True if there is an empty space in the given column.
    # Otherwise returns False.
    if column < 0 or column >= (BOARDWIDTH) or board[column][0] != EMPTY: # if the column is outside the gameboard (left or right of it) or there isn't an empty space,
...
def isBoardFull(board): # has the entire board been filled?
    ...
    for x in range(BOARDWIDTH): # checking each x coordinate (column) in the game board
        for y in range(BOARDHEIGHT): # checking each y coordinate (row) in the game board
...
def isWinner(board, tile): # checks for 4 in a row
    ...
    for x in range(BOARDWIDTH - 3): # starting from the right most column, keep checking columns to the left until the fourth column
        for y in range(BOARDHEIGHT): # checking each row
            if board[x][y] == tile and board[x+1][y] == tile and board[x+2][y] == tile and board[x+3][y] == tile: # keep checking each tile to the left. If they correspond with the tile specified,
                return True # it's a four in a row!
              
    # check vertical spaces
    for x in range(BOARDWIDTH): # checking each column,
        for y in range(BOARDHEIGHT - 3): # starting from the bottom most row, keep checking rows above until the fourth row
            if board[x][y] == tile and board[x][y+1] == tile and board[x][y+2] == tile and board[x][y+3] == tile: # keep checking each tile above. If they correspond with the tile specified,
                return True # it's a four in a row!
              
    # check / diagonal spaces
    for x in range(BOARDWIDTH - 3): # starting from the right most column, keep checking columns to the left until the fourth column
        for y in range(3, BOARDHEIGHT): # checking from the thrid row to the sixth from the bottom
            if board[x][y] == tile and board[x+1][y-1] == tile and board[x+2][y-2] == tile and board[x+3][y-3] == tile: # keep checking each tile to the left and down. If they correspond with the tile specified,
                return True # it's a four in a row!
              
    # check \ diagonal spaces
    for x in range(BOARDWIDTH - 3): # starting from the right most column, keep checking columns to the left until the fourth column
        for y in range(BOARDHEIGHT - 3): # starting from the bottom most row, keep checking rows above until the fourth row
            if board[x][y] == tile and board[x+1][y+1] == tile and board[x+2][y+2] == tile and board[x+3][y+3] == tile: # keep checking each tile to the left and up. If they correspond with the tile specified,
                return True # it's a four in a row!
...
if __name__ == '__main__': # if "__main__" is the name of the file, run the file.
    main() # if the variable __name__ is set as "__main__" then the interpreter will run the module containing main()
